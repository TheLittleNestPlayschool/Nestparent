...
function getArchiveGroups(){
  ...
  media.filter(item=>!item.is_deleted).forEach(item=>{
    const date=getMediaDate(item.created_at);
    if(!date||date>=currentMonthStart)return;
...
/*   create archive card*/
export function createMemoryArchiveCard({onArchive}={}){
  ...
  article.querySelectorAll(".memory-archive-item").forEach(button=>{
    button.addEventListener("click",event=>{
      ...
      window.dispatchEvent(new CustomEvent("parent:memory-archive",{detail:payload}));
    });
  });
