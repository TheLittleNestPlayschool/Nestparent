...
export function createMemoryViewerCard({
  ...
  article.innerHTML=`
    <div class="memory-viewer-stage-card">
      ...
      <div class="memory-viewer-top">
        <span class="memory-viewer-kicker">${getCollectionLabel(collection)}</span>
        <span class="memory-viewer-counter"></span>
      </div>
      <button class="memory-viewer-nav memory-viewer-prev" type="button" aria-label="Previous memory">‹</button>
      <button class="memory-viewer-nav memory-viewer-next" type="button" aria-label="Next memory">›</button>
      <div class="memory-viewer-bottom">
        ...
        <div class="memory-viewer-actions">
          <button class="memory-viewer-download" type="button">
            <span class="memory-viewer-download-symbol">↓</span>
            <span class="memory-viewer-download-label">Download</span>
          </button>
          <button class="memory-viewer-share" type="button">
            <span class="memory-viewer-share-symbol">↗</span>
            <span>Share</span>
          </button>
        </div>
      </div>
  ...
  const mediaContainer=article.querySelector(".memory-viewer-media");
  const counter=article.querySelector(".memory-viewer-counter");
  const date=article.querySelector(".memory-viewer-date");
  const kind=article.querySelector(".memory-viewer-kind");
  const previous=article.querySelector(".memory-viewer-prev");
  const next=article.querySelector(".memory-viewer-next");
  const download=article.querySelector(".memory-viewer-download");
  const share=article.querySelector(".memory-viewer-share");
  ...
  function renderCurrent(){
    ...
    kind.textContent=item.media_kind==="video"?"Video":"Photo";
    previous.disabled=currentIndex===0;
    next.disabled=currentIndex===mediaItems.length-1;
    share.hidden=item.sharable!==true;
  }
  ...
  download.addEventListener("click",async event=>{
    event.stopPropagation();
    await downloadMemoryMedia(mediaItems[currentIndex],download);
  });
  ...
  share.addEventListener("click",event=>{
    event.stopPropagation();
    const item=mediaItems[currentIndex];
    if(!item||item.sharable!==true)return;
