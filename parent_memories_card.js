...
const specialCollectionPresentation={
  recognition:{title:"Recognition Days",symbol:"✦"},
  birthday:{title:"Birthdays",symbol:"♡"}
};
...
function getMemoryMedia(){
  ...
  return media.map((item,index)=>({...item,thumbnail:thumbnails[index]||""})).filter(item=>!item.is_deleted);
}
...
function getLiveMemoryCollections(){
  ...
  const validMediaIds=new Set(media.map(item=>Number(item.id)));
  return collectionTypes.filter(type=>type.is_active).map(type=>{
    const code=String(type.code||"").trim().toLowerCase();
    const matchingMediaIds=new Set(collectionLinks.filter(link=>Number(link.media_collection_type_id)===Number(type.id)&&validMediaIds.has(Number(link.student_media_id))).map(link=>Number(link.student_media_id)));
    if(matchingMediaIds.size===0){return null;}
    const presentation=specialCollectionPresentation[code]||{};
    return{id:code||String(type.id),typeId:type.id,code,title:presentation.title||type.name,symbol:presentation.symbol||"✦",count:matchingMediaIds.size};
  }).filter(Boolean);
}
...
export function createMemoriesCard({onChapter,onCollection}={}){
  const article=document.createElement("article");
  const memories=getLiveMemories();
  const chapters=getLiveChapters();
  article.className="experience memories-experience";
  article.dataset.type="memories";
  article.innerHTML=`
    <div class="memories-stage-card">
      <div class="memories-card-heading">
        <h2 class="memories-title">${memories.title}</h2>
      </div>
      <div class="memories-hero">
        <div class="memories-photo" style="background-image:url('${memories.photo}')"></div>
        <div class="memories-photo-shade"></div>
        <div class="memories-heading">
          <div class="memories-count">${memories.count} ${memories.count===1?"little moment saved":"little moments saved"}</div>
  ...
  /*   chapter actions*/
  article.querySelectorAll(".memory-chapter").forEach(button=>{
    button.addEventListener("click",event=>{
      event.stopPropagation();
      const chapter=button.dataset.memoryChapter;
      const handled=typeof onChapter==="function"&&onChapter(chapter)===true;
      if(handled){return;}
      window.dispatchEvent(new CustomEvent("parent:memory-chapter",{detail:{chapter}}));
    });
  });

  /*   special collection actions*/
  article.querySelectorAll(".memory-special").forEach(button=>{
    button.addEventListener("click",event=>{
      event.stopPropagation();
      const collection=button.dataset.memoryCollection;
      const handled=typeof onCollection==="function"&&onCollection(collection)===true;
      if(handled){return;}
      window.dispatchEvent(new CustomEvent("parent:memory-collection",{detail:{collection}}));
    });
  });
  return article;
}
